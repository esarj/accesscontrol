...
export class AccessControl {
    ...
    removeRoles(roles) {
        if (this.isLocked)
            throw new AccessControlError(ERR_LOCK);
        let rolesToRemove = utils.toStringArray(roles);
        ...
        utils.eachRole(this._grants, (roleItem, roleName) => {
    ...
    getInheritedRolesOf(role) {
        let roles = utils.getRoleHierarchyOf(this._grants, role);
        roles.shift();
        return roles;
    }
    /**
    ...
    _removePermission(resources, roles, actionPossession) {
        ...
        utils.eachRoleResource(this._grants, (role, resource, resourceInfo) => {
