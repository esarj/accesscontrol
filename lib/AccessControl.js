...
import { Access, Query, Permission, AccessControlError } from './core/index.js';
...
export class AccessControl {
    ...
    removeRoles(roles) {
        if (this.isLocked)
            throw new AccessControlError(ERR_LOCK);
        const rolesToRemove = utils.toStringArray(roles);
        ...
        utils.eachRole(this._grants, (roleItem, _roleName) => {
    ...
    getInheritedRolesOf(role) {
        const roles = utils.getRoleHierarchyOf(this._grants, role);
        roles.shift();
        return roles;
    }
    /**
    ...
    _removePermission(resources, roles, actionPossession) {
        ...
        utils.eachRoleResource(this._grants, (role, resource, _resourceInfo) => {
            if (resources.indexOf(resource) >= 0
                ...
                if (actionPossession) {
                    ...
                    const ap = utils.normalizeActionPossession({ action: actionPossession }, true);
