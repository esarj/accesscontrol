...
const utils = {
    ...
    validRoleObject(grants, roleName) {
        ...
        if (!role || utils.type(role) !== 'object') {
            throw new index_js_2.AccessControlError(`Invalid role definition.`);
        }
        utils.eachKey(role, (resourceName) => {
            if (!utils.validName(resourceName, false)) {
                if (resourceName === '$extend') {
                    let extRoles = role[resourceName]; // semantics
    ...
    getResources(grants) {
        ...
        let resources = {};
        utils.eachRoleResource(grants, (role, resource, permissions) => {
    ...
    normalizeActionPossession(info, asString = false) {
        ...
        if (index_js_1.actions.indexOf(s[0].trim().toLowerCase()) < 0) {
        ...
        if (poss) {
            if (index_js_1.possessions.indexOf(poss.trim().toLowerCase()) < 0) {
    ...
    normalizeQueryInfo(query) {
        ...
        if (!utils.isFilledStringArray(query.role)) {
            throw new index_js_2.AccessControlError(`Invalid role(s): ${JSON.stringify(query.role)}`);
        }
        // validate resource
        if (typeof query.resource !== 'string' || query.resource.trim() === '') {
            throw new index_js_2.AccessControlError(`Invalid resource: "${query.resource}"`);
        }
    ...
    normalizeAccessInfo(access, all = false) {
        ...
        if (access.role.length === 0 || !utils.isFilledStringArray(access.role)) {
            throw new index_js_2.AccessControlError(`Invalid role(s): ${JSON.stringify(access.role)}`);
        }
        ...
        if (access.resource.length === 0 || !utils.isFilledStringArray(access.resource)) {
            throw new index_js_2.AccessControlError(`Invalid resource(s): ${JSON.stringify(access.resource)}`);
        }
    ...
    getRoleHierarchyOf(grants, roleName, rootRole) {
        ...
        role.$extend.forEach((exRoleName) => {
            if (!grants[exRoleName]) {
                throw new index_js_2.AccessControlError(`Role not found: "${grants[exRoleName]}"`);
            }
    ...
    lockAC(ac) {
        ...
        if (!locked) {
            throw new index_js_2.AccessControlError(`Could not lock grants: ${typeof _ac._grants}`);
        }
