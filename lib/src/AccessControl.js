...
class AccessControl {
    ...
    setGrants(grantsObject) {
        if (this.isLocked)
            throw new index_js_1.AccessControlError(utils_js_1.ERR_LOCK);
        this._grants = utils_js_1.utils.getInspectedGrants(grantsObject);
        return this;
    }
    /**
    ...
    extendRole(roles, extenderRoles) {
        if (this.isLocked)
            throw new index_js_1.AccessControlError(utils_js_1.ERR_LOCK);
        utils_js_1.utils.extendRole(this._grants, roles, extenderRoles);
        return this;
    }
    /**
    ...
    removeRoles(roles) {
        if (this.isLocked)
            throw new index_js_1.AccessControlError(utils_js_1.ERR_LOCK);
        let rolesToRemove = utils_js_1.utils.toStringArray(roles);
        if (rolesToRemove.length === 0 || !utils_js_1.utils.isFilledStringArray(rolesToRemove)) {
            throw new index_js_1.AccessControlError(`Invalid role(s): ${JSON.stringify(roles)}`);
        }
        rolesToRemove.forEach((roleName) => {
            if (!this._grants[roleName]) {
                throw new index_js_1.AccessControlError(`Cannot remove a non-existing role: "${roleName}"`);
            }
            delete this._grants[roleName];
        });
        // also remove these roles from $extend list of each remaining role.
        utils_js_1.utils.eachRole(this._grants, (roleItem, roleName) => {
            if (Array.isArray(roleItem.$extend)) {
                roleItem.$extend = utils_js_1.utils.subtractArray(roleItem.$extend, rolesToRemove);
            }
        });
        return this;
    }
    ...
    getInheritedRolesOf(role) {
        let roles = utils_js_1.utils.getRoleHierarchyOf(this._grants, role);
        roles.shift();
        return roles;
    }
    /**
     ...
     */
    getResources() {
        return utils_js_1.utils.getResources(this._grants);
    }
    /**
    ...
    can(role) {
        ...
        if (arguments.length !== 0 && role === undefined) {
            throw new index_js_1.AccessControlError('Invalid role(s): undefined');
        }
        // other explicit invalid values will be checked in constructor.
        return new index_js_1.Query(this._grants, role);
    }
    /**
     *  Alias of `can()`.
     *  @private
    ...
    permission(queryInfo) {
        return new index_js_1.Permission(this._grants, queryInfo);
    }
    /**
    ...
    grant(role) {
        ...
        // other explicit invalid values will be checked in constructor.
        return new index_js_1.Access(this, role, false);
    }
    /**
     *  Alias of `grant()`.
     *  @private
    ...
    deny(role) {
        ...
        // other explicit invalid values will be checked in constructor.
        return new index_js_1.Access(this, role, true);
    }
    /**
     *  Alias of `deny()`.
     *  @private
    ...
    _removePermission(resources, roles, actionPossession) {
        resources = utils_js_1.utils.toStringArray(resources);
        // resources is set but returns empty array.
        if (resources.length === 0 || !utils_js_1.utils.isFilledStringArray(resources)) {
            throw new index_js_1.AccessControlError(`Invalid resource(s): ${JSON.stringify(resources)}`);
        }
        if (roles !== undefined) {
            roles = utils_js_1.utils.toStringArray(roles);
            // roles is set but returns empty array.
            if (roles.length === 0 || !utils_js_1.utils.isFilledStringArray(roles)) {
                throw new index_js_1.AccessControlError(`Invalid role(s): ${JSON.stringify(roles)}`);
            }
        }
        utils_js_1.utils.eachRoleResource(this._grants, (role, resource, permissions) => {
            if (resources.indexOf(resource) >= 0
                ...
                if (actionPossession) {
                    ...
                    const ap = utils_js_1.utils.normalizeActionPossession({ action: actionPossession }, true);
     ...
     */
    static get Error() {
        return index_js_1.AccessControlError;
    }
    // -------------------------------
    ...
    static filter(data, attributes) {
        return utils_js_1.utils.filterAll(data, attributes);
    }
    /**
    ...
    static isACError(object) {
        return object instanceof index_js_1.AccessControlError;
    }
    /**
     *  Alias of `isACError`
     *  @private
