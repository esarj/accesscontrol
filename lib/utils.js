...
export const utils = {
     ...
     */
    iterateArray(arrayInput, callbackFn, thisContext = null) {
        const len = arrayInput.length;
        let idx = -1;
        while (++idx < len) {
            if (callbackFn.call(thisContext, arrayInput[idx], idx, arrayInput) === false)
                break;
        }
    },
    /**
     ...
     */
    iterateObjectKeys(objectInput, callbackFn, thisContext = null) {
    ...
    eachRole(grants, callback) {
        utils.iterateObjectKeys(grants, (name) => callback(grants[name], name));
    },
    /**
     *
     */
    eachRoleResource(grants, callback) {
        let roleInfo;
        let resourceInfo;
        utils.iterateObjectKeys(grants, (role) => {
            roleInfo = grants[role];
            utils.iterateObjectKeys(roleInfo, (resource) => {
    ...
    hasValidNames(list, throwOnInvalid = true) {
        let allValid = true;
        utils.iterateArray(utils.toStringArray(list), (name) => {
    ...
    validResourceObject(o) {
        ...
        const obj = o;
        utils.iterateObjectKeys(obj, (action) => {
    ...
    validRoleObject(grants, roleName) {
        ...
        utils.iterateObjectKeys(role, (resourceName) => {
            if (!utils.validName(resourceName, false)) {
                if (resourceName === '$extend') {
                    const extRoles = role.$extend ?? []; // semantics
    ...
    getInspectedGrants(o) {
        ...
        if (type === 'object') {
            utils.iterateObjectKeys(o, (roleName) => {
    ...
    getResources(grants) {
        ...
        const resources = {};
        utils.eachRoleResource(grants, (role, resource, _resourceInfo) => {
    ...
    extendRole(grants, roles, extenderRoles) {
        ...
        roles.forEach((roleName) => {
            ...
            if (crossInherited) {
                throw new AccessControlError(`Cross inheritance is not allowed. Role "${crossInherited}" already ` +
                    `extends "${roleName}".`);
            }
    ...
    commitToGrants(grants, access, normalizeAll = false) {
        ...
        accessInfo.role.forEach((role) => {
            ...
            const ap = accessInfo.action + ':' + accessInfo.possession;
     ...
     */
    // Hatanın işaret ettiği fonksiyon burasıydı.
    // Hem parametrelerin hem de dönüş tipinin açıkça belirtildiğinden emin oluyoruz.
    filterAll(data, attributes // Hata 2: "typedef" - tipin `string[]` olduğu belirtildi.
    ) {
        if (_.isArray(data)) {
            // `map` fonksiyonu bir dizi döndüreceği için dönüş tipiyle uyumlu.
            return data.map((o) => this.filter(o, attributes));
        }
        // `filter` fonksiyonu bir nesne döndüreceği için dönüş tipiyle uyumlu.
