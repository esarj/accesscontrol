'use strict';
Object.defineProperty(exports, "__esModule", { value: true });
exports.helper = void 0;
/**
 *  Test Helper
 *  @author   Onur Yıldırım <onur@cutepilot.com>
 */
const src_1 = require("../src");
const core_1 = require("../src/core");
const helper = {
    expectACError(fn, errMsg) {
        expect(fn).toThrow();
        try {
            fn();
        }
        catch (err) {
            expect(err instanceof src_1.AccessControl.Error).toEqual(true);
            expect(err instanceof core_1.AccessControlError).toEqual(true);
            expect(src_1.AccessControl.isAccessControlError(err)).toEqual(true);
            expect(src_1.AccessControl.isACError(err)).toEqual(true); // alias test
            if (errMsg && typeof err === 'object' && err !== null && 'message' in err && typeof err.message === 'string') {
                expect(err.message).toContain(errMsg);
            }
        }
    }
};
exports.helper = helper;
//# sourceMappingURL=helper.js.map