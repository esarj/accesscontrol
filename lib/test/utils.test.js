...
describe('Test Suite: utils (generic)', () => {
    ...
    test('#isFilledStringArray(), #isEmptyArray()', () => {
        expect(utils_1.utils.isFilledStringArray([])).toBe(true); // allowed
    ...
    test('#each(), #eachKey()', () => {
        ...
        const o = { x: 0, y: 1, z: 2 };
        const d = {}; // Corrected: Added explicit type
        ...
        utils_1.utils.each([1], function (item) {
            expect(this.ok).toBe(true);
        }, new Context());
        utils_1.utils.eachKey({ key: 1 }, function (key) {
            expect(this.ok).toBe(true);
        }, new Context());
    });
});
describe('Test Suite: utils (core)', () => {
    test('#validName(), #hasValidNames()', () => {
        ...
        const invalid = utils_1.RESERVED_KEYWORDS[0];
        ...
        expect(utils_1.utils.hasValidNames(validList)).toBe(true);
        expect(utils_1.utils.hasValidNames(validList, false)).toBe(true);
        const invalidList = ['valid', utils_1.RESERVED_KEYWORDS[utils_1.RESERVED_KEYWORDS.length - 1]];
        helper_1.helper.expectACError(() => utils_1.utils.hasValidNames(invalidList));
        expect(utils_1.utils.hasValidNames(invalidList, false)).toBe(false);
    });
    ...
    test('#normalizeQueryInfo(), #normalizeAccessInfo()', () => {
        // Corrected: Cast invalid inputs to 'any' for testing purposes
        helper_1.helper.expectACError(() => utils_1.utils.normalizeQueryInfo(null));
        helper_1.helper.expectACError(() => utils_1.utils.normalizeQueryInfo({ role: null }));
        ...
        helper_1.helper.expectACError(() => utils_1.utils.normalizeAccessInfo(null));
        helper_1.helper.expectACError(() => utils_1.utils.normalizeAccessInfo({ role: null }));
    ...
    test('#getRoleHierarchyOf()', () => {
        const grants = { 'admin': { '$extend': ['user'] } };
        ...
        // Corrected: Cast invalid inputs to 'any' for testing purposes
        helper_1.helper.expectACError(() => utils_1.utils.getRoleHierarchyOf(grants, null));
        helper_1.helper.expectACError(() => utils_1.utils.getRoleHierarchyOf(grants, ''));
    });
    test('#getFlatRoles()', () => {
        // Corrected: Cast invalid inputs to 'any' for testing purposes
        helper_1.helper.expectACError(() => utils_1.utils.getFlatRoles({}, null));
    ...
    test('#getCrossExtendingRole()', () => {
        const grants = { 'user': {}, 'admin': { '$extend': ['user', 'editor'] }, 'editor': { '$extend': ['admin'] } };
        // Corrected: Cast invalid inputs to 'any' for testing purposes
        expect(utils_1.utils.getCrossExtendingRole(grants, 'admin', 'admin')).toEqual('admin');
    ...
    test('#extendRole()', () => {
        const grants = { 'user': {}, 'admin': { '$extend': ['user', 'editor'] }, 'editor': { '$extend': ['admin'] }, 'viewer': {} };
        // Corrected: Cast invalid inputs to 'any' for testing purposes
        helper_1.helper.expectACError(() => utils_1.utils.extendRole(grants, null, ['admin']));
        helper_1.helper.expectACError(() => utils_1.utils.extendRole(grants, 'admin', null));
        ...
        helper_1.helper.expectACError(() => utils_1.utils.extendRole(grants, 'admin', 'editor')); // cross
        helper_1.helper.expectACError(() => utils_1.utils.extendRole(grants, '$', 'user')); // reserved keyword
        expect(() => utils_1.utils.extendRole(grants, 'admin', 'viewer')).not.toThrow();
    });
    test('#getUnionAttrsOfRoles()', () => {
        const grants = { 'user': { 'account': { 'read:own': ['*'] } }, 'admin': { '$extend': ['user'] } };
        const query = { role: 'admin', resource: 'account', action: 'read' };
    ...
    test('#lockAC()', () => {
        // Corrected: Cast invalid inputs to 'any' for testing purposes
        expect(() => utils_1.utils.lockAC(null)).toThrow();
