...
describe('Test Suite: AccessControl', () => {
    ...
    test('reset grants with #reset() only', () => {
        ...
        helper.expectACError(() => (ac as unknown).setGrants());
    ...
    test('add grants from flat list (db), check/remove roles and resources', () => {
        ...
        expect((ac as unknown).hasRole()).toEqual(false);
        ...
        expect((ac as unknown).hasResource()).toEqual(false);
    ...
    test('#removeResources(), #_removePermission()', () => {
        ...
        (ac as unknown).removeResources('photo', 'user');
        expect(ac.can('admin').createAny('photo').granted).toEqual(true);
        ...
        helper.expectACError(() => (ac as unknown)._removePermission(null));
        helper.expectACError(() => (ac as unknown)._removePermission(''));
        helper.expectACError(() => (ac as unknown)._removePermission([]));
        helper.expectACError(() => (ac as unknown)._removePermission(['']));

        grantAll();
        helper.expectACError(() => (ac as unknown)._removePermission('photo', ''));
        helper.expectACError(() => (ac as unknown)._removePermission(['photo'], null));
        helper.expectACError(() => (ac as unknown)._removePermission('photo', []));
        helper.expectACError(() => (ac as unknown)._removePermission('photo', ['']));

        // passing the third argument (actionPossession)
        grantAll();
        (ac as unknown)._removePermission('photo', 'user', 'create');
        expect(ac.can('admin').createAny('photo').granted).toEqual(true);
        expect(ac.can('user').createAny('photo').granted).toEqual(false);
        expect(ac.can('user').createOwn('photo').granted).toEqual(true);
    ...
    test('explicit undefined', () => {
        const ac = new AccessControl();
        helper.expectACError(() => (ac as unknown).grant(undefined));
        helper.expectACError(() => (ac as unknown).deny(undefined));
        helper.expectACError(() => (ac as unknown).can(undefined));
        helper.expectACError(() => (ac as unknown).query(undefined));
    });
    ...
    test('throw on invalid or reserved names', () => {
        ...
        helper.expectACError(() => (ac as unknown).grant(1));
        helper.expectACError(() => (ac as unknown).grant(true));
        helper.expectACError(() => (ac as unknown).grant(false));
        helper.expectACError(() => (ac as unknown).grant([]));
        helper.expectACError(() => (ac as unknown).grant({}));
    ...
    test('AccessControl#lock(), Access#lock()', () => {
        ...
        function _inoperative() {
            ...
            expect(() => (ac as unknown)._grants.hacker = { account: { 'read:any': ['*'] } }).toThrow();
        ...
        Object.freeze((ac as unknown)._grants);
        ac.lock();
